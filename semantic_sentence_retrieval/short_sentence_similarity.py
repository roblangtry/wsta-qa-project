...
# http://sujitpal.blogspot.com.au/2014/12/semantic-similarity-for-short-sentences.html

# Sentence Similarity Based on Semantic Nets and Corpus Statistics
# Yuhua Li, David McLean, Zuhair A. Bandar, James D. O'Shea, and Keeley Crockett
# Paper http://ants.iis.sinica.edu.tw/3BkMJ9lTeWXTSrrvNoKNFDxRm3zFwRR/55/Sentence%20Similarity%20Based%20on%20Semantic%20Nets%20and%20corpus%20statistics.pdf
...
def semantic_tokens_similarity(tokens_1, tokens_2, info_content_norm):
    """
    Computes the semantic similarity between two sentences as the cosine
    similarity between the semantic vectors computed for each sentence.
    """
    joint_words = set(tokens_1).union(set(tokens_2))
    vec_1 = semantic_vector(tokens_1, joint_words, info_content_norm)
    vec_2 = semantic_vector(tokens_2, joint_words, info_content_norm)
...
# Reuben
def word_order_tokens_similarity(tokens_1, tokens_2):
    """
    Computes the word-order similarity between two sentences as the normalized
    difference of word order between the two sentences.
    """
    joint_words = list(set(tokens_1).union(set(tokens_2)))
    windex = {x[1]: x[0] for x in enumerate(joint_words)}
    r1 = word_order_vector(tokens_1, joint_words, windex)
    r2 = word_order_vector(tokens_2, joint_words, windex)
    return 1.0 - (np.linalg.norm(r1 - r2) / np.linalg.norm(r1 + r2))
...
#Reuben
def tokens_similarity(tokens_1, tokens_2, info_content_norm):
    """
    Calculate the semantic similarity between two sentences. The last
    parameter is True or False depending on whether information content
    normalization is desired or not.
    """
    return DELTA * semantic_tokens_similarity(tokens_1, tokens_2, info_content_norm) + \
        (1.0 - DELTA) * word_order_tokens_similarity(tokens_1, tokens_2)
